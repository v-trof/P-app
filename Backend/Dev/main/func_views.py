...
def create_course(request):
    if request.method == 'POST':
        ...
        course = Course.objects.create_course(
            name=name, subject=subject, creator=creator)
        course.save()
        if request.user.courses:
            setattr(request.user, 'courses',
                    request.user.courses + " " + str(course.id))
        else:
            setattr(request.user, 'courses', str(course.id))
        request.user.save()
        db.commit()
...
def invite_students(request):
    if request.method == 'POST':
        ...
        text_content_nonreg = 'Вам поступило приглашение на курс ' + course.name + ' от ' + request.user.name + \
            ' . Перейдите по ссылке для регистрации на курс 127.0.0.1:8000/register/' + \
            str(course.id)
        text_content = 'Вам поступило приглашение на курс ' + course.name + ' от ' + request.user.name + \
        ...
        for value in email_list:
            ...
            if User.objects.filter(email=value):
                msg = EmailMultiAlternatives(
                    subject, text_content, from_email, [value])
                msg.send()
            else:
                msg = EmailMultiAlternatives(
                    subject, text_content_nonreg, from_email, [value])
                msg.send()
        return HttpResponse("ok")
...
def invite_teacher(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(email=email):
            ...
            msg = EmailMultiAlternatives(
                subject, text_content_nonreg, from_email, [email])
            msg.send()
        return HttpResponse("ok")
            ...
            if request.user.email not in data["pending_users"]["Заявки"]:
                ...
                for group in data["pending_users"]:
                    if request.user.email in data["pending_users"][group]:
                        ...
                        data["groups"][group][request.user.id]={}
                        data["groups"][group][request.user.id]["unseen_by"]=len(data["teachers"])
                        data["pending_users"][group].remove(request.user.email)
                        for teacher in data["teachers"]:
                            print (teacher)
                            data["teachers"][teacher]["new_users"].append(request.user.id)
                if not data["status"] == "closed" and not is_invited:
                    group = "Нераспределенные"
                    data["users"].append(request.user.id)
                    data["groups"][group].append(request.user.id)
                    data["groups"][group][request.user.id]["unseen_by"]=len(course["teachers"])
                    for teacher in data["teachers"]:
                        data["teachers"][teacher]["new_users"].append(request.user.id)
                elif data["status"] == "closed":
                    data["pending_users"]["Заявки"].append(request.user.id)
            with io.open('courses/' + str(course.id) + '/info.json', 'w', encoding='utf8') as json_file:
                saving_data = json.dumps(data, ensure_ascii=False)
                json_file.write(saving_data)
    return redirect('/course/' + str(course_id) + '/groups/')
#course[users][user.id][unseen_by]--
#course[teachers][request.user.id].remove(user.id)
    ...
    if course_id:
        ...
        for course in user.courses.split(' '):
            ...
            with io.open('courses/' + str(course.id) + '/info.json', 'r', encoding='utf8') as data_file:
                ...
                user_data[course.id]["updates"]["new_students"]=[]
                for user_id in data["teachers"][str(request.user.id)]["new_users"]:
                    user_data[course.id]["updates"]["new_students"].append(User.objects.get(id=user_id))
                ...
                print(user_data[course.id]["updates"])
    return user_data
