...
def overall_test(request):
	...
	functions["Course"]["Decline_request"]=course_decline_request(user=user, course=course)
	...
	#functions["Test"]["Set_mark_quality"]=test_set_mark_quality(mark=mark)
	functions["Test"]["Get_results"]=test_get_results(course_id=course_id, test_id=test_id)
	functions["Test"]["Get_test_info"]=test_get_test_info(course_id=course_id, test_id=test_id)
	functions["Test"]["Get_attempt_info"]=test_get_test_info(course_id=course_id, test_id=test_id,user=user)
...
def test_get_results(course_id,test_id):
	response = Test.objects.get_results(course_id=course_id,test_id=test_id)
	if response or response==0 :
		return "OK"
	else : return "ERROR"
